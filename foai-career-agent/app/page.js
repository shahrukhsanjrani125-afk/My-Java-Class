      ...
      <div className="mx-auto flex min-h-screen max-w-7xl flex-col px-6 py-8">
        ...
        <section className="grid flex-1 items-center gap-10 py-16 lg:grid-cols-2">
          <div>
            ...
            "Skill Gap",
            "Interview Prep",
        ].map((item) => (<span key={item} className="rounded-full border border-slate-700 bg-slate-900 px-4 py-2 text-sm text-slate-300">
                  {item}
                </span>))}
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-slate-800 bg-slate-900 p-6 shadow-2xl">
            ...
            <div className="flex gap-3">
              <input value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Ask about your career..." className="min-w-0 flex-1 rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm outline-none placeholder:text-slate-600 focus:border-cyan-400"/>

              <button onClick={() => setMessage("")} className="rounded-xl bg-cyan-400 px-5 py-3 text-sm font-semibold text-slate-950 transition hover:bg-cyan-300">
                Send
              </button>
            </div>
          </div>
