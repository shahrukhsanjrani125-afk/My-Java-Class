...
# Built-in Autonomous Web Terminal Dashboard
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>SHARK VIP - Autonomous 24/7 Terminal</title>
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Courier New', Courier, monospace; }
        ...
        .terminal-header { background: #1a2236; padding: 12px 15px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #00ffcc; }
...
<body>
    <div class="terminal-container">
        <header class="terminal-header">
            <span class="title">🦈 SHARK_AUTONOMOUS_BOT [24/7 INFINITE LOOP]</span>
            <span id="live-clock"></span>
        </header>
        <div class="terminal-body">
            <div class="status">[⚡] Status: System running autonomously. Auto-dispatching to Telegram...</div>
            <ul id="signal-list"><li>Initializing signal generation engine...</li></ul>
        </div>
    </div>
        ...
        async function fetchLiveSignals() {
            try {
                ...
                data.signals.forEach(sig => {
                    ...
                    li.innerHTML = `<strong>${sig.asset}</strong> | ⏰ ${sig.time} | 📈 <strong>${sig.direction}</strong><br><small style="color: #8892b0;">Market: ${sig.marketType.toUpperCase()} | ${sig.spec}</small>`;
                    list.appendChild(li);
                });
            } catch (err) {
...
"""
            ...
            # Direct dispatch (Ensure your PC is connected via a VPN if local ISP blocks Telegram)
...
if __name__ == '__main__':
    # Start background daemon thread for 24/7 infinite loop automation
    daemon_thread = threading.Thread(target=background_infinite_loop, daemon=True)
    daemon_thread.start()
    
    print("==================================================")
    print(" SHARK 24/7 AUTONOMOUS BOT STARTED SUCCESSFULLY")
    print(" Web Dashboard Link: http://localhost:5000")
    print("==================================================")
